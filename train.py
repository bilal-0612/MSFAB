...
datasets = ['data/offline-train.pkl', 'data/train-caption.txt']
valid_datasets = ['data/offline-2019-test.pkl', 'data/test-caption-2019.txt']
...
if use_pretrianed_model:
    ...
    match = re.search(r'epoch(\d+)', pretrained_model_path)
    model = load_checkpoint_part_weight(model, pretrained_model_path)
...
for eidx in (range(max_epochs)):
    ...
    for x, y in tqdm(train):
        ...
        if np.mod(uidx, dispFreq) == 0:
            ...
            loss_s2/=dispFreq
            loss_kl/=dispFreq
            # print('Data',datetime.now().strftime('%Y-%m-%d %H-%M-%S'),'Epoch',eidx, 'Update/total: ',str(uidx)+'/'+str(len(train)), 
                #   'Cost_L2R: ', loss_s,'Cost_R2L: ', loss_s2,'KL: ', loss_kl, 'time ', ud_s, 'lrate ', lrate, 'eps', my_eps, 'bad_counter', bad_counter)
            ud_s = 0
            loss_s = 0.
            loss_s2=0
            loss_kl=0
        ...
        if np.mod(uidx, validFreq) == 0 and valid_stop == False :
            ...
            fpp_write = open(result_path+'/train_valid_record.txt','a')
            fpp_write.write('\n'+str(eidx)+'\t'+str(valid_err)+'\t'+str(valid_sacc))
            #fpp_write.write(str(eidx))
            #fpp_write.write('\t')
            #fpp_write.write(str(valid_err))
            #fpp_write.write('\t')
            #fpp_write.write(str(valid_sacc))
            fpp_write.close()
