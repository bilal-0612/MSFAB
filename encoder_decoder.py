...
class Encoder_Decoder(nn.Module):
    ...
    def forward(self, params, x, x_mask, y, y_mask, y_reverse,y_mask_reverse,one_step=False):
        ...
        ctx_mask = x_mask
        ctx1 , encoder1_dens2_out= self.encoder1(x)
        ...
        if params['L2R'] == 1:        
            ...
            h2ts1, cts1, alphas1, _alpha_pasts = self.gru_model(params, emb1, y_mask, ctx1, encoder1_dens2_out, ctx_mask, one_step, init_state1, alpha_past=None)
        ...
        if params['R2L'] == 1:
            ...
            h2ts2, cts2, alphas2, _alpha_pasts = self.gru_model2(params, emb2, y_mask_reverse, ctx1, encoder1_dens2_out, ctx_mask, one_step, init_state2, alpha_past=None)
    ...
    def f_init(self, x, x_mask=None,idx_decoder=1):
        ...
        return init_state1,ctx1,encoder1_dens2_out


    # decoding: decoder part
    def f_next(self, params, y, y_mask, ctx, ctx_high_res, ctx_mask, init_state, alpha_past, one_step,idx_decoder=1):
        ...
        if idx_decoder == 1:
            ...
            next_state, cts, _alpha, next_alpha_past = self.gru_model(params, emb_beam, y_mask, ctx, ctx_high_res, ctx_mask, one_step, init_state, alpha_past)
            ...
            # one step of two gru layers
            next_state, cts, _alpha, next_alpha_past = self.gru_model2(params, emb_beam, y_mask, ctx, ctx_high_res, ctx_mask, one_step, init_state, alpha_past)
            # reshape to suit GRU step code
            next_state_ = next_state.view(1, next_state.shape[0], next_state.shape[1])
            cts = cts.view(1, cts.shape[0], cts.shape[1])
            emb_beam = emb_beam.view(1, emb_beam.shape[0], emb_beam.shape[1])
