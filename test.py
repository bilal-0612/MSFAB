...
def main(beam_k,model_path, dictionary_target, fea, latex, saveto, output, idx_decoder, year):
    ...
    model = load_checkpoint_part_weight(model, model_path)
    model.cuda()
    model.eval()
    ...
    # load data
    test, test_uid_list = dataIterator(fea, latex, worddicts, batch_size=4, batch_Imagesize=500000, maxlen=20000, maxImagesize=500000)
...
if __name__ == "__main__":
    ...
    assert args.year in years, "Valid years are only 2014, 2016 and 2019"
    
    main(args.k, args.model_path, args.dictionary_target, args.test_dataset, args.label, args.saveto, args.output, args.idx_decoder, args.year)
