...
def load_weights(model, weights_path):
    print("Loading model: ", model.year)
    weights = torch.load("2019 model/2019.pkl", map_location=lambda storage, loc: storage, weights_only=True)
    model.load_state_dict(weights)
    return model

import copy

def clean_state_dict(state_dict, layers_to_remove):
    """
    Removes specified layers from a PyTorch state_dict and returns a new, cleaned state_dict.

    Args:
        state_dict (dict): A dictionary of parameter names (keys) -> tensors (values).
        layers_to_remove (list): A list of strings indicating parameter keys to remove.
    
    Returns:
        dict: A new state_dict with the specified keys removed.
    """
    # Make a copy so the original state_dict is not modified
    cleaned_dict = copy.deepcopy(state_dict)

    # Remove the unwanted layers
    for layer_name in layers_to_remove:
        if layer_name in cleaned_dict:
            del cleaned_dict[layer_name]
...
def gen_sample_bidirection(model, x, params, gpu_flag, k=1, maxlen=30, idx_decoder=1):
    ...
    if gpu_flag:
        next_state, ctx0, ctx0_high_res = model.f_init(x,idx_decoder=idx_decoder)
    else:
        next_state, ctx0, ctx0_high_res = model.f_init(x,idx_decoder=idx_decoder)
    ...
    ctx0 = ctx0.cpu().numpy()
    ctx0_high_res = ctx0_high_res.cpu().numpy()
    ...
    for ii in range(maxlen):
        ...
        ctx_high_res = np.tile(ctx0_high_res, [live_k, 1, 1, 1])
        ctx_high_res = torch.from_numpy(ctx_high_res).cuda()
        if gpu_flag:
            next_p, next_state, next_alpha_past,alpha = model.f_next(params, next_w, None, ctx, ctx_high_res, None, next_state,
...
def load_checkpoint_part_weight(model, checkpoint_path):
    ...
    if checkpoint_path and os.path.isfile(checkpoint_path):
        checkpoints    = torch.load(checkpoint_path, map_location='cuda')
        checkpoints    = checkpoints[model.year]
