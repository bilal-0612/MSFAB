...
class Gru_cond_layer_aam(nn.Module):
    def __init__(self, params):
        ...
        self.conv_Ub = nn.Conv2d(600, params['dim_attention'], kernel_size=1, stride=2)
    ...
    def forward(self, params, embedding, mask=None, context=None, high_res_feat=None, context_mask=None, one_step=False, init_state=None,
                alpha_past=None):
...
#            Ub_ctx = self.conv_Ub(high_res_feat)
#            Ub_ctx = Ub_ctx.permute(2, 3, 0, 1)
        Ub_ctx = self.conv_Ub(high_res_feat)
        Ub_ctx = Ub_ctx.permute(2, 3, 0, 1)
        ...
        if one_step:
            ...
            h2ts, cts, alphas, alpha_pasts = self._step_slice(mask, state_below_r, state_below_z, state_below_h,
                                                              init_state, context, context_mask, alpha_past, Ua_ctx, Ub_ctx)
        else:
    ...
    def _step_slice(self, mask, state_below_r, state_below_z, state_below_h, h, ctx, ctx_mask, alpha_past, Ua_ctx, Ub_ctx = None):
        # the first GRU layer
...
#        if(Ub_ctx is not None):
#            Ua_ctx = Ua_ctx + Ub_ctx
            
        attention_score = torch.tanh(Ua_ctx + Wa_h1[None, None, :, :] + cover_vector+cover_vector2 + Ub_ctx)
